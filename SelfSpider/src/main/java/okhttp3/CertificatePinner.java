 ...
 * and read the expected configuration when the connection fails. Be sure to do
 * this on a trusted network, and without man-in-the-middle tools like <a
 ...
 * 	CertificatePinner certificatePinner = new CertificatePinner.Builder().add( hostname, &quot;sha1/AAAAAAAAAAAAAAAAAAAAAAAAAAA=&quot; ).build();
 ...
 * {
 * 	&#064;code
 * 	CertificatePinner certificatePinner = new CertificatePinner.Builder().add( &quot;publicobject.com&quot;, &quot;sha1/DmxUShsZuNiqPQsX2Oi9uv2sCnw=&quot; )
 * 			.add( &quot;publicobject.com&quot;, &quot;sha1/SXxoaOSEzPC6BgGmxAt/EAcsajw=&quot; ).add( &quot;publicobject.com&quot;, &quot;sha1/blhOM3W9V/bVQhsWAcLYwPU6n24=&quot; )
 ...
 * {@code publicobject.com} and {@code www.publicobject.com}, you must configure
 * both hostnames.
 * <p>
 * <p>
 * Wildcard pattern rules:
 * <ol>
 * <li>Asterisk {@code *} is only permitted in the left-most domain name label
 * and must be the only character in that label (i.e., must match the whole
 * left-most label). For example, {@code *.example.com} is permitted, while
 * {@code *a.example.com}, {@code a*.example.com}, {@code a*b.example.com},
 * {@code a.*.example.com} are not permitted.
 ...
 * </ol>
 * <p>
 * If hostname pinned directly and via wildcard pattern, both direct and
 * wildcard pins will be used. For example: {@code *.example.com} pinned with
 * {@code pin1} and {@code a.example.com} pinned with {@code pin2}, to check
 * {@code a.example.com} both {@code pin1} and {@code pin2} will be used.
 * <p>
 ...
 * <p>
 * <p>
 * {@link CertificatePinner} can not be used to pin self-signed certificate if
 * such certificate is not accepted by {@link javax.net.ssl.TrustManager}.
 *
 * @see <a
...
public final class CertificatePinner {
     ...
     * {@code hostname} is in {@code peerCertificates}. Does nothing if there
     * are no certificates pinned for {@code hostname}. OkHttp calls this after
     * a successful TLS handshake, but before the connection is used.
     *
    ...
    /**
     * Returns list of matching certificates' pins for the hostname or
     * {@code null} if hostname does not have pinned certificates.
     */
    ...
    public static final class Builder {
         ...
         *
         * @param hostname lower-case host name or wildcard pattern such as
         *                 {@code *.example.com}.
         * @param pins     SHA-1 hashes. Each pin is a SHA-1 hash of a certificate's
         *                 Subject Public Key Info, base64-encoded and prefixed with
         *                 {@code sha1/}.
         */
