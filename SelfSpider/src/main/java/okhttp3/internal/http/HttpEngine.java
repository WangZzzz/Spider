...
public final class HttpEngine {
    ...
    /**
     * @param request                 the HTTP request without a body. The body must be written via
     ...
     */
    public HttpEngine(OkHttpClient client, Request request, boolean bufferRequestBody, boolean callerWritesRequestBody, boolean forWebSocket,
                      StreamAllocation streamAllocation, RetryableSink requestBodyOut, Response priorResponse) {
    ...
    private static Address createAddress(OkHttpClient client, Request request) {
        ...
        return new Address(request.url().host(), request.url().port(), client.dns(), client.socketFactory(), sslSocketFactory, hostnameVerifier,
                certificatePinner, client.proxyAuthenticator(), client.proxy(), client.protocols(), client.connectionSpecs(), client.proxySelector());
    }
     ...
     * @throws IOException      if there was a problem while making a request. Sometimes
    ...
    public void sendRequest() throws RequestException, RouteException, IOException {
        ...
        if (networkRequest == null && cacheResponse == null) {
            userResponse = new Response.Builder().request(userRequest).priorResponse(stripBody(priorResponse)).protocol(Protocol.HTTP_1_1).code(504)
        ...
        if (networkRequest == null) {
            userResponse = cacheResponse.newBuilder().request(userRequest).priorResponse(stripBody(priorResponse))
            ...
            if (writeRequestHeadersEagerly()) {
                ...
                if (bufferRequestBody) {
                    ...
                    if (contentLength != -1) {
                        ...
                        // write request headers until the
                        // entire body is ready; otherwise we can't set the
    ...
    /**
     * If the caller's control flow writes the request body, we need to create
     * that stream immediately. And that means we need to immediately write the
     * request headers, so we can start streaming the request body. (We may
     * already have a request body if we're retrying a failed POST.)
     */
    private boolean writeRequestHeadersEagerly() {

        return callerWritesRequestBody && permitsRequestBody(networkRequest) && requestBodyOut == null;
    }

    private HttpStream connect() throws RouteException, RequestException, IOException {

        boolean doExtensiveHealthChecks = !networkRequest.method().equals("GET");
        return streamAllocation.newStream(client.connectTimeoutMillis(), client.readTimeoutMillis(), client.writeTimeoutMillis(),
    ...
    public HttpEngine recover(IOException e, Sink requestBodyOut) {
        ...
        // connection.
        return new HttpEngine(client, userRequest, bufferRequestBody, callerWritesRequestBody, forWebSocket, streamAllocation,
    ...
    private Request networkRequest(Request request) throws IOException {
        ...
        if (cookies != null && !cookies.isEmpty()) {
            // CMBCLog.d( TAG, request.url().toString() + " cookies: " +
    ...
    public void readResponse() throws IOException {
        ...
        if (cacheResponse != null) {
            if (validate(cacheResponse, networkResponse)) {
                userResponse = cacheResponse.newBuilder().request(userRequest).priorResponse(stripBody(priorResponse))
                        .headers(combine(cacheResponse.headers(), networkResponse.headers())).cacheResponse(stripBody(cacheResponse))
        ...
        userResponse = networkResponse.newBuilder().request(userRequest).priorResponse(stripBody(priorResponse))
    ...
    private Response readNetworkResponse() throws IOException {
        ...
        Response networkResponse = httpStream.readResponseHeaders().request(networkRequest).handshake(streamAllocation.connection().handshake())
                .header(OkHeaders.SENT_MILLIS, Long.toString(sentRequestMillis))
                .header(OkHeaders.RECEIVED_MILLIS, Long.toString(System.currentTimeMillis())).build();

        if (!forWebSocket) {
            networkResponse = networkResponse.newBuilder().body(httpStream.openResponseBody(networkResponse)).build();
        }
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        Source cacheWritingSource = new Source() {
            ...
            public void close() throws IOException {
                ...
                if (!cacheRequestClosed && !discard(this, HttpStream.DISCARD_STREAM_TIMEOUT_MILLIS, MILLISECONDS)) {
                    cacheRequestClosed = true;
                    cacheRequest.abort();
                }
                source.close();
     ...
     * engine's response. This will either add authentication headers, follow
